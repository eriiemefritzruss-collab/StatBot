...
class StatBot:
    def __init__(self, config_path='config.yaml'):
        ...
        load_env_file(os.path.join(bundle_dir, ".env"))
        config_path = os.path.join(bundle_dir, config_path)

        with open(config_path, 'r') as f:
            self.config = yaml.load(f, Loader=yaml.FullLoader)
        self.resolve_env_config(["api_key"])
        ...
        self.conv.programmer.messages = [
            {
                "role": "system",
                "content": self.get_programmer_prompt()
            }
        ]
    ...
    def resolve_env_config(self, keys):
        for key in keys:
            value = self.config.get(key)
            if isinstance(value, str) and value.startswith("${") and value.endswith("}"):
                env_name = value[2:-1]
                self.config[key] = os.environ.get(env_name, "")

    def get_programmer_prompt(self):
        if self.config.get("analysis_profile") == "statistics":
            return STAT_PROGRAMMER_PROMPT.format(working_path=self.session_cache_path)
        return PROGRAMMER_PROMPT.format(working_path=self.session_cache_path)

    def open_board(self):
        return self.conv.show_data()

    def _resolve_uploaded_file_path(self, files):
        if files is None:
            raise ValueError("No file was uploaded.")

        # Gradio may provide a tempfile-like object, a filepath string, or a list.
        ...
        raise TypeError(f"Unsupported uploaded file object: {type(files)}")

    def add_file(self, files):
        try:
            file_path = self._resolve_uploaded_file_path(files)
            filename = os.path.basename(file_path)
            local_cache_path = os.path.join(self.session_cache_path, filename)

            shutil.copy(file_path, local_cache_path)
            file_extension = os.path.splitext(file_path)[1].lower()
            if filename not in self.conv.file_list:
            ...
            if file_extension in ['.csv', '.xlsx', '.xls']:
                ...
                self.conv.session_state.register_upload(filename, gen_info)
                ...
                status_message = (
                    f"### Upload succeeded\n"
                    f"- File: `{filename}`\n"
                    f"- Rows: {gen_info['num_rows']}\n"
                    f"- Columns: {gen_info['num_features']}"
                )
            else:
                self.conv.programmer.messages[0][
                    "content"] += f"\nNow, user uploads the files in {local_cache_path}."
                status_message = f"### Upload succeeded\n- File: `{filename}`"

            print(f"Upload file in gradio path: {file_path}, local cache path: {local_cache_path}")
            return status_message
        except Exception as e:
            print(f"Failed to handle uploaded file: {e}")
            raise gr.Error(f"Upload failed: {e}")

    def rendering_code(self):
        return self.conv.rendering_code() or "# No code has been generated in this session yet."

    def generate_report(self, chat_history):
        try:
            down_path = self.conv.document_generation(chat_history)
            status_message = f"### Report generated\n- File: `{os.path.basename(down_path)}`"
            return [
                gr.Button(visible=False),
                gr.DownloadButton(label="Download Report", value=down_path, visible=True),
                status_message,
            ]
        except Exception as e:
            print(f"Report generation failed: {e}")
            raise gr.Error(f"Report generation failed: {e}")
    ...
    def chat_streaming(self, message, chat_history, code=None):
        ...
        if not code:
            if not message or not str(message).strip():
                raise gr.Error("Please enter a question before submitting.")
